...
<div class="bg-[#FFE4DB] min-h-screen" x-data="{ mobileMenuOpen: false }">
    <div class="flex h-screen p-2 md:p-4 gap-4 md:gap-6 overflow-hidden relative">
        ...
        <main class="flex-1 overflow-y-auto pr-1 md:pr-2 custom-scrollbar">
            ...
            <!-- Stat Cards -->
            <div class="grid grid-cols-1 sm:grid-cols-2 {{ Auth::user() && Auth::user()->role === 'Admin' ? 'lg:grid-cols-3 xl:grid-cols-5' : 'lg:grid-cols-4' }} gap-4 mb-6">
                ...
                <!-- Total Revenue — always last -->
                <div class="bg-white p-5 rounded-2xl shadow-sm border border-orange-50 group hover:border-emerald-400 transition-all cursor-default">
                    <p class="text-gray-500 text-xs font-semibold uppercase tracking-wider">Total Revenue</p>
                    <div class="flex items-end justify-between mt-2">
                        <h3 class="text-2xl font-bold text-gray-800">${{ number_format($totalRevenue, 2) }}</h3>
                        <div class="p-2 bg-emerald-50 text-emerald-500 rounded-lg group-hover:bg-emerald-400 group-hover:text-white transition-colors">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
