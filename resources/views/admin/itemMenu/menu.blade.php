...
<div class="bg-[#FFE4DB] min-h-screen"
    ...
    <div x-show="mobileMenuOpen" @click="mobileMenuOpen = false"
        class="fixed inset-0 bg-black/50 z-40 xl:hidden no-print"></div>
            ...
            <div class="bg-[#EE6D3C] px-6 pt-6 pb-5 relative flex-shrink-0">
                ...
                <div class="text-center">
                    ...
                    <h3 class="text-white text-2xl font-black"
                        x-text="'Order #' + (viewOrder ? viewOrder.id : '')"></h3>
                    <p class="text-white/70 text-xs mt-1"
                        x-text="viewOrder ? (viewOrder.created_at + ' · ' + viewOrder.time) : ''"></p>
                </div>
                <div class="mt-3 bg-white/15 rounded-2xl py-3 px-4 text-center">
                    ...
                    <p class="text-white font-black text-3xl leading-tight"
                        x-text="'$' + parseFloat(viewOrder ? viewOrder.total_amount : 0).toFixed(2)"></p>
                    <p class="text-white/60 text-xs mt-0.5"
                        x-text="'≈ ៛' + toKhr(viewOrder ? viewOrder.total_amount : 0)"></p>
                </div>
            </div>
                    ...
                    <div class="divide-y divide-gray-50 max-h-[200px] overflow-y-auto custom-scrollbar">
                        ...
                        <template x-for="(item, idx) in (viewOrder ? viewOrder.items : [])" :key="idx">
                            <div class="flex justify-between items-center px-4 py-3 text-sm">
                                ...
                                <span class="inline-flex items-center justify-center w-6 h-6 bg-orange-100 text-orange-700 text-xs font-bold rounded-full"
                                    x-text="item.quantity"></span>
                            </div>
                        </template>
                    </div>
        ...
        <div class="bg-white w-full max-w-sm rounded-3xl shadow-2xl relative overflow-hidden max-h-[95vh] flex flex-col">
            ...
            <div class="bg-[#EE6D3C] px-6 pt-6 pb-5 relative flex-shrink-0">
                ...
                <div class="text-center">
                    <p class="text-white/70 text-xs uppercase tracking-widest font-semibold mb-0.5">FastBite</p>
                    <h3 class="text-white text-2xl font-black"
                        x-text="'Invoice #' + (invoiceOrder ? invoiceOrder.id : '')"></h3>
                    <p class="text-white/70 text-xs mt-1"
                        x-text="invoiceOrder ? (invoiceOrder.created_at + ' · ' + invoiceOrder.time) : ''"></p>
                </div>
                <div class="mt-3 bg-white/15 rounded-2xl py-3 px-4 text-center">
                    <p class="text-white/70 text-xs font-semibold uppercase tracking-widest mb-0.5">Total Amount</p>
                    <p class="text-white font-black text-3xl leading-tight"
                        x-text="'$' + parseFloat(invoiceOrder ? invoiceOrder.total_amount : 0).toFixed(2)"></p>
                    <p class="text-white/60 text-xs mt-0.5"
                        x-text="'≈ ៛' + toKhr(invoiceOrder ? invoiceOrder.total_amount : 0)"></p>
                </div>
            </div>
            ...
            <div class="px-6 py-5 overflow-y-auto custom-scrollbar flex-1">
                ...
                <div class="text-center mb-5">
                    <p class="text-xs text-gray-500 font-bold uppercase tracking-widest mb-2">
                        🇰🇭 Scan to Pay — NBC Bakong KHQR
                    </p>
                    <div class="inline-flex flex-col items-center bg-white border-2 border-dashed border-[#003087]/25 rounded-2xl p-3">
                        ...
                        <img x-show="khqrDataUrl && !khqrLoading"
                             :src="khqrDataUrl"
                             alt="Bakong KHQR Payment QR"
                        ...
                        <div class="mt-2 text-center">
                            <p class="font-black text-[#EE6D3C] text-lg"
                               x-text="'$' + parseFloat(invoiceOrder ? invoiceOrder.total_amount : 0).toFixed(2)"></p>
    ...
    <div id="printable-invoice" style="display:none;">
        <div style="font-family: 'Courier New', monospace; max-width: 380px; margin: 0 auto;">
            ...
            <div style="text-align:center; border-bottom: 2px solid #000; padding-bottom:12px; margin-bottom:16px;">
                ...
                <p style="font-size:13px; font-weight:700; margin:6px 0 0;"
                    x-text="invoiceOrder ? 'Invoice #' + invoiceOrder.id : 'Receipt'"></p>
                <p style="font-size:11px; color:#777; margin:3px 0 0;"
                    x-text="invoiceOrder ? (invoiceOrder.created_at + '  ·  ' + invoiceOrder.time) : ''"></p>
                <p style="font-size:11px; margin:3px 0 0;"
                    x-text="invoiceOrder ? 'Status: ' + (invoiceOrder.status.charAt(0).toUpperCase() + invoiceOrder.status.slice(1)) : ''"></p>
            </div>
