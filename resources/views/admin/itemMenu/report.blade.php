        ...
        <div class="bg-white rounded-lg shadow-sm border border-orange-100 p-6 md:p-8 mt-4 mb-8">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-xs font-bold bg-[#FFE4DB] text-[#EE6D3C] px-3 py-1 rounded-full" x-text="activeTab.charAt(0).toUpperCase() + activeTab.slice(1)"></span>
                </div>
                <button onclick="window.print()"
                    class="flex items-center gap-2 bg-gray-900 hover:bg-gray-700 text-white text-sm font-bold px-4 py-3 rounded-xl transition no-print">
            ...
            <div class="flex gap-1 bg-gray-100 rounded-xl p-1 mb-6 no-print overflow-x-auto no-scrollbar">
                <template x-for="tab in ['reservations','orders','products','categories']" :key="tab">
                    <button
                        @click="activeTab = tab"
                        :class="activeTab === tab
                            ? 'bg-white text-[#EE6D3C] shadow font-bold'
                        ...
                        <tbody class="divide-y">
                            ...
                            <tr class="hover:bg-orange-50/50 transition"
                                ...
                                <td class="p-4 hidden md:table-cell">
                                    <span>{{ $res->date }}</span>
                                    <div class="text-xs text-gray-400">{{ $res->time }}</div>
                                </td>
                ...
                <div class="w-full overflow-x-auto rounded-xl border border-gray-100">
                    <table class="w-full text-left text-sm">
                        ...
                        <tbody class="divide-y">
                            ...
                            <tr class="hover:bg-orange-50/50 transition"
                                x-show="matchOrder('{{ $order->customer_name ?? $order->id }} {{ $order->status }}', '{{ $order->status }}')">
                                ...
                                <td class="p-4 hidden md:table-cell font-semibold text-gray-800">
                    ...
                    <table class="w-full text-left text-sm">
                        ...
                        <tbody class="divide-y">
                            ...
                            <tr class="hover:bg-orange-50/50 transition"
                                ...
                                <td class="p-4 hidden lg:table-cell text-gray-600">
                                    {{ $product->stock ?? 'N/A' }}
                                </td>
                            </tr>
                            @empty
                    ...
                    <table class="w-full text-left text-sm">
                        ...
                        <tbody class="divide-y">
                            ...
                            <tr class="hover:bg-orange-50/50 transition"
                                ...
                                <td class="p-4 hidden lg:table-cell text-gray-500 text-xs">
                                    {{ \Carbon\Carbon::parse($cat->created_at)->format('d M Y') }}
                                </td>
                            </tr>
                            @empty
        ...
        <table class="w-full text-left border-collapse mb-8">
            ...
            <tbody>
                ...
                <tr class="border-b">
                    ...
                    <td class="py-2">{{ $order->customer_name ?? 'Guest' }}</td>
                    <td class="py-2">{{ \Carbon\Carbon::parse($order->created_at)->format('d M Y') }}</td>
                    <td class="py-2">${{ number_format($order->total_price ?? $order->total ?? 0, 2) }}</td>
        ...
        <table class="w-full text-left border-collapse mb-8">
            ...
            <tbody>
                ...
                <tr class="border-b">
                    ...
                    <td class="py-2">{{ $product->category?->name ?? 'Uncategorized' }}</td>
                    <td class="py-2">${{ number_format($product->price, 2) }}</td>
                    <td class="py-2">{{ $product->stock ?? 'N/A' }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
        ...
        <table class="w-full text-left border-collapse mb-8">
            ...
            <tbody>
                ...
                <tr class="border-b">
                    ...
                    <td class="py-2">{{ $cat->products_count }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
